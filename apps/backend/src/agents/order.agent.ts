...
export class OrderAgent {
  static async execute(
    ...
    if (!targetOrderId) {
      ...
      const response = `${intro}\n\n${orderList}\n\n*You can ask me to track, cancel, return, or check details for any of these orders by mentioning the Order ID (e.g. ${orders[0]?.orderNumber || "ORDER-1001"}).*`;
    ...
    // 3. Specific Order Operations: Return, Cancel, Track, Details
    ...
    reasoningSteps.push({
      ...
      stage: "tool_execution",
      agent: "ORDER",
      thought: `Executed tool ${toolName} for ${targetOrderId}. Result: ${toolResult.success ? "Success" : "Notice"}.`,
      timestamp: now(),
      toolCall: toolCallRecord,
    });
    ...
    if (toolResult.success) {
      ...
      response = `### Return / Order Notification: **${targetOrderId}**\n\nHello **${user.name}**, ${toolResult.message}`;
    }
