...
export class BillingAgent {
  static async execute(
    ...
    const specificOrderId = context.currentEntities.orderNumbers[0];
    ...
    reasoningSteps.push({
      ...
      stage: "analyzing",
      agent: "BILLING",
      thought: `Analyzing billing request for customer ${user.name}. Target Invoice: ${specificInvoiceId || "All"}, Order: ${specificOrderId || "N/A"}.`,
      timestamp: now(),
    });

    // If no specific invoice or refund target is given and user asks for invoices
    if (!specificInvoiceId && !specificOrderId && (lower.includes("invoice") || lower.includes("bills") || lower.includes("payments") || lower.includes("charges") || context.isAccountLevelQuery)) {
      ...
      const response = `Hello **${user.name}**, here are the invoices recorded for your account (*${user.email}*):\n\n${invoiceList}\n\n*You can ask me to inspect an invoice or process a refund by mentioning the Invoice ID (e.g. INV-2024-001).*`;
    ...
    if (lower.includes("refund") && (lower.includes("request") || lower.includes("want") || lower.includes("process"))) {
      ...
      toolResult = await billingTools.checkRefundStatus({ query: specificInvoiceId || specificOrderId || "INV-2024-002" });
    ...
    reasoningSteps.push({
      ...
      stage: "tool_execution",
      agent: "BILLING",
      thought: `Queried billing ledger using ${toolName}.`,
      timestamp: now(),
      toolCall: toolCallRecord,
    });
    ...
    if (toolResult.success) {
      if (toolName === "checkRefundStatus") {
        ...
        response = `### Active Subscriptions & Invoicing\n\nHello **${user.name}**, ${toolResult.message}\n\nAll subscriptions automatically bill on the 1st of each calendar month.`;
      } else {
        const d = toolResult.data;
      ...
      response = `${toolResult.message} If you have a specific Invoice ID (e.g. INV-2024-001) or Order ID, please provide it so I can locate your payment ledger.`;
    }
